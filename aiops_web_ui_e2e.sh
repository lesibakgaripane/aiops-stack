...
echo "Using WEB UI script at: ${SCRIPT_PATH}"
...
if [ "$CODE1" = "200" ]; then
  if ! grep -qi "Welcome to nginx" /tmp/ui_root_local.html 2>/dev/null; then
    ...
    echo "⚠️  UI root is default nginx page (check nginx config)"
  fi
else
...
if [ "$CODE" = "200" ]; then
  echo "✅ /ui-api/chat is reachable and returned HTTP 200 (treated as healthy)."
else
  echo "⚠️  /ui-api/chat did not return HTTP 200 (see body above for details)."
fi
echo "------------------------------------------------------"

echo "✅ WEB UI E2E CHECK COMPLETED (no hard failures flagged here)."
